...
//Add By 01↓
		function check_display($privileges = array(), $app, $act = 'all')
		{
			$mp_disp = $app . '|' . $act;
			...
			else
			{
				...
				if (in_array($app . '|all', $privs) || in_array($mp_disp, $privs))
				{
					return true;
				}
		...
		foreach ((array)($menu) as $handlekey => $handle) {
			if ($handlekey != 'dashboard') {
				foreach ((array)$handle['children'] as $menukey => $menus) {
					$url_index = explode('index.php?', $menus['url']);
					$url_and = explode('&', $url_index[1]);
					$url_app = explode('app=', $url_and[0]);
					$url_act = explode('act=', $url_and[1]);
					$app_url = $url_app[1];
					$act_url = $url_act[1];
					if (!check_display($this->visitor->get('privs'), $app_url, $act_url)) {
						unset($menu[$handlekey]['children'][$menukey]);
						unset($back_nav[$handlekey]['children'][$menukey]);
					}
				}
			}
		}
