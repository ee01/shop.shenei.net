...
class MemberApp extends MemberbaseApp
{
    ...
    function register()
    {
        ...
        if (!IS_POST)
        {
            ...
            /* 导入jQuery的表单验证插件 */
            $this->import_resource('mlselection.js, jquery.plugins/jquery.validate.js');	//Modify By 01
        ...
        else
        {
			...
			if($_POST['consignee']) $regdata['real_name']=$_POST['consignee'];	//Add By 01
			if($_POST['phone_mob']) $regdata['phone_mob']=$_POST['phone_mob'];	//Add By 01
...
//			$user_id = $ms->user->register($user_name, $password, $email);
            $user_id = $ms->user->register($user_name, $password, $email, $regdata);	//Modify By 01
			...
			if ($_POST['consignee'] && $_POST['region_id'] && $_POST['address']) {
