...
class FrontendApp extends ECBaseApp
{
    ...
    {
        $cart =& m('cart');
//		$this->assign('cart_goods_kinds', $cart->get_kinds(SESS_ID, $this->visitor->get('user_id')));	//$this->visitor这个对象未正确实例化
		$this->assign('cart_goods_kinds', is_object($cart) && is_object($this->visitor) ? $cart->get_kinds(SESS_ID, $this->visitor->get('user_id')) : 0);	//Modify By 01	加入对变量的检查
...
class MemberbaseApp extends MallbaseApp
{
    ...
    function _get_member_menu()
    {
        ...
        $menu['mymps'] = array(
            ...
            'submenu'   => array(
                'post_mps'  => array(
                    'text'  => Lang::get('post_mps'),
                    'url'   => 'http://xx.shenei.net/member/info.php?part=input',
                    'name'  => 'post_mps',
                    'icon'  => 'ico10',
                ),
                'my_mps'  => array(
                    'text'  => Lang::get('my_mps'),
                    'url'   => 'http://xx.shenei.net/member/info.php?part=all',
                    'name'  => 'my_mps',
                    'icon'  => 'ico5',
                ),
                'mps_msg'  => array(
                    'text'  => Lang::get('mps_msg'),
                    'url'   => 'http://xx.shenei.net/member/pm.php',
                    'name'  => 'mps_msg',
                    'icon'  => 'ico3',
                ),
                'mps_contact'  => array(
                    'text'  => Lang::get('mps_contact'),
                    'url'   => 'http://xx.shenei.net/member/update.php?part=contact',
                    'name'  => 'mps_contact',
                    'icon'  => 'ico7',
                ),
            ),
