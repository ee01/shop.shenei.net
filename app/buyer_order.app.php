...
class Buyer_orderApp extends MemberbaseApp
{
    ...
    function _get_orders()
    {
        ...
        $order_type =& ot('groupbuy');	//Add By 01
        foreach ($orders as $key1 => $order)
        {
			...
			//Add By 01↓
			$orders[$key1]['order_logs'] = $order_type->_get_order_logs($key1);
            foreach ($orders[$key1]['order_logs'] as $key3 => $logs) {
				$lastorder = $key3;
            }
			$orders[$key1]['order_logs_lastreason'] = $orders[$key1]['order_logs'][$lastorder];
			//Add By 01↑
        }
