...
//            'limit'   =>$page['limit'],	//用数组剪切	Modify By 01
        ...
        foreach ($stores as $key => $store)
        {
			...
			//重新排序	Add By 01
			if (empty($_GET['order']) || !in_array($_GET['order'], array('credit_value desc'))) {
				$re_order[$key] = $store['sort_order'];
			}else{
				$re_order[$key] = $store['credit_value'];
			}
			$re_order_goods[$key] = $stores[$key]['goods_count'];

        }

		//重新排序	Add By 01
		if (empty($_GET['order']) || !in_array($_GET['order'], array('credit_value desc'))) {
			array_multisort($re_order, SORT_ASC,$re_order_goods, SORT_DESC, $stores);
		}else{
			array_multisort($re_order, SORT_DESC,$re_order_goods, SORT_DESC, $stores);
		}
		$stores = array_slice($stores,empty($_REQUEST['page']) ? 0 : (intval($_REQUEST['page'])-1)*10,10);
    ...
    function mymps()
    {
		//global $keyword;
		$keyword = isset($_GET['keyword']) ? trim($_GET['keyword']) : '';
		header("Location:http://xx.shenei.net/public/info.php?part=search&keywords=".$keyword);
    }
}
