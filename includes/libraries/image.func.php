...
//Add By 01↓	限定图片最大尺寸
		...
		if($maxtoh < 300) $maxtoh = 1;
		$maxtowh = $maxtow/$maxtoh;
		$srcwh = $src_width/$src_height;
		if($maxtowh <= $srcwh){
			$fmaxtow = $maxtow;
			$fmaxtoh = $fmaxtow*($src_height/$src_width);
		} else {
			$fmaxtoh = $maxtoh;
			$fmaxtow = $fmaxtoh*($src_width/$src_height);
		}
		...
		if($src_width <= $maxtow && $src_height <= $maxtoh) {
			$make_max = 0;	//不处理
		}
		
		if($make_max) {
			$maxni = $func_imagecreate($fmaxtow, $fmaxtoh);
			$func_imagecopy($maxni, $im, 0, 0, 0, 0, $fmaxtow, $fmaxtoh, $src_width, $src_height);
			imagejpeg($maxni, $src, $quality);
		}
//Add By 01↑
